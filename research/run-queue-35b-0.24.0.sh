#!/usr/bin/env bash
# Tune queue — vLLM 0.24.0, RedHatAI/Qwen3.6-35B-A3B-NVFP4 (MoE A3B + hybrid attn + MTP).
# ABLATION: each candidate = the 0.24.0 baseline + ONE change, measured (N=3, c1 sentinel +
# c16 objective) vs the same baseline. Variants are pre-generated on disk (runbooks/, committed).
# Smoke-gated per run (run_experiment); numeric-risky knobs (MoE/linear GEMM backend, KV dtype)
# also get an mmlu eval (LIMIT=100, matched to the baseline reference). Does NOT auto-promote —
# writes a report with the recommendation; promotion stays a reviewed step (validate.sh + promote.sh).
set -uo pipefail   # NOT -e: keep going past a failed candidate
cd "$(dirname "$0")/.."
RBDIR=runbooks/RedHatAI/Qwen3.6-35B-A3B-NVFP4
BASE="$RBDIR/20260704_v0.24.0_baseline.sh"
RESDIR=results/gb10-1988a9714b4e/RedHatAI/Qwen3.6-35B-A3B-NVFP4
SUM="$RESDIR/OVERNIGHT-35b-0.24.0.md"
MMLU_REF="${MMLU_REF:-78.19}"   # baseline mmlu (LIMIT=100) — quality ref for numeric-risky knobs (override w/ the measured 0.24 baseline mmlu)
...
log "=== 35B tune queue start (vLLM 0.24.0) ==="
log "measuring 0.24.0 baseline (N=3, c1/c16)"
BL=$(measure "$BASE"); BASE_C16=$(parse c16 "$BL"); log "baseline: $BL"
[ -z "${BASE_C16:-}" ] && BASE_C16=344.6   # fall back to the recorded 0.23 baseline c16

run_candidate(){
  local slug="$1" risky="$2"
  local rb="$RBDIR/20260704_${slug}_tuned.sh"
  [ -f "$rb" ] || { log "MISSING variant $rb — skip"; RESULT[$slug]="na|na|missing|variant file absent"; return; }
  log "--- candidate: $slug ($(sed -n 's/^# Deltas vs base: //p' "$rb" | head -1)) ---"
  git add "$rb" 2>/dev/null && git commit -q -m "exp(35b/0.24.0): $slug" 2>/dev/null
  ...
  # numeric-risky (GEMM backend / KV dtype): if it served, eval mmlu at the matched LIMIT and compare recovery
  if [ "$risky" = 1 ] && [ "$st" = ok ]; then
    log "$slug numeric-risky -> mmlu eval (LIMIT=100)"
    if scripts/serve.sh "$rb" >/tmp/q35_24_serve.log 2>&1; then
      TASKS=mmlu LIMIT=100 scripts/eval.sh "$rb" general >/tmp/q35_24_eval.log 2>&1 || log "eval errored (see /tmp/q35_24_eval.log)"
...
# Report
{
  echo "# 35B tune queue — vLLM 0.24.0, Qwen3.6-35B-A3B-NVFP4 ($(date -u +%Y-%m-%dT%H:%M:%SZ))"
  echo
  echo "Baseline (0.24.0, flashinfer_cutlass MoE + fp8 KV + MTP n=1) median **c16 = ${BASE_C16}** tok/s"
  ...
  echo "scripts/validate.sh + scripts/promote.sh VLLM_TAG=24 -> VLLM-24-RedHatAI_Qwen3.6-35B-A3B_NVFP4_final.sh;"
  echo "else the 0.24 baseline stands. Independent wins can be combined into one variant + re-validated."
} > "$SUM"
git add -A && git commit -q -m "35b tune queue (0.24.0): ablation results + report" 2>/dev/null
