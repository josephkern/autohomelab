...
# Gate 2 — quality (general reference + resistant tier-2)
gen=ok;  ensure_up && "$SCRIPT_DIR/eval.sh" "$RUNBOOK" general   || gen=error;  log "Gate 2 general: $gen"
res=ok;  ensure_up && "$SCRIPT_DIR/eval.sh" "$RUNBOOK" resistant || res=error;  log "Gate 2 resistant: $res"

# Gate 3 — throughput, full curve, both shapes (one bench.sh call amortizes the rest of the session)
bench=ok
