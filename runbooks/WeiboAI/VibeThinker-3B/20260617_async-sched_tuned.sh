#!/usr/bin/env bash
# TUNED VARIANT (async-sched) — base: runbooks/WeiboAI/VibeThinker-3B/baseline.sh
# Deltas vs base: + `--async-scheduling`.
# Hypothesis: overlap scheduler with GPU exec to cut per-step bubbles; compute-bound 3B scales to c32,
#   so any scheduling slack hidden could lift batched c16.
