#!/usr/bin/env bash
# TUNED VARIANT (ctx262144) — base: runbooks/unsloth/Qwen3.8-27B-NVFP4/VLLM-27-unsloth_Qwen3.8-27B_NVFP4_final.sh
# Deltas vs base: --max-model-len 8192 -> 262144
# WHY: the promoted _final serves 8192 against a model whose native context is 262144 — 1/32nd of
#   its capability. That value came from gen_baseline.py and rode through FOUR tuning waves
#   untouched, because it is not a throughput knob and both benchmark shapes (chat 512/256, coder
#   4096/1024) fit inside it, so nothing ever pushed against the ceiling.
# WHAT THIS MEASURES, and what it does NOT: the chat shape uses ~768 tokens per sequence, so KV
#   demand is unchanged by this flag. What CAN change is engine-side: KV pool sizing, CUDA-graph
#   capture shapes, and the scheduler's token budget. So a flat c16 here means 'raising the limit
#   is FREE for short-context serving' — it does NOT mean long-context serving is free.
#   The real long-context cost is arithmetic, not a benchmark: max concurrent sequences =
#   (GPU KV cache tokens) / (context actually used). Capture the 'GPU KV cache size' line from the
#   serve log for each config and compute it — that is the number a deployment decision needs.
# Risk: may serve_fail if the KV pool cannot hold one max-length sequence. The 8192 baseline
#   reported ~371k KV tokens at util 0.5, so 262144 should just fit, but MTP reserves extra.
#   Numerically identical either way, so Gate 2 carries over.
