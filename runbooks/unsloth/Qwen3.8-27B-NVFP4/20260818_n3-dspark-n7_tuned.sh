#!/usr/bin/env bash
# TUNED VARIANT (n3-dspark-n7) — base: runbooks/unsloth/Qwen3.8-27B-NVFP4/20260818_mtp-n3_tuned.sh
# Deltas vs base: speculative-config mtp/n=3 -> dspark drafter RadixArk/Qwen3.8-27B-DSpark, n=7.
# Hypothesis: a separate 1.36B BF16 drafter (already downloaded, 2.6 GB) whose card publishes a MEAN
#   ACCEPTED LENGTH of 3.39 (per-workload 2.71-4.57) at block size 7. Our MTP n=3 emits roughly 1.9
#   tokens per target pass (c1 25.64 against a 13.60 no-spec baseline). Past a bandwidth wall, accepted
#   LENGTH is the currency, so if anything close to 3.39 transfers this is the largest lever left.
#   DSpark's marginal draft cost is also reported far below MTP's, which is what makes depth 7 affordable.
# MISMATCH TO WATCH: the drafter was trained against Qwen/Qwen3.8-27B-FP8; our target is this NVFP4
#   build. Same base model and hidden geometry (verified field-by-field), so it should LOAD, but a
#   drafter keys off target hidden states and NVFP4 numerics may cost acceptance. That is the question.
# Risk: HIGH + NUMERIC-RISKY. DSpark is a different drafter architecture (block-diffusion + a confidence
#   head choosing draft length DYNAMICALLY), which is not obviously covered by the standard
#   rejection-sampling guarantee, and our charter records ds4's DSpark as divergent under greedy decode.
#   A KEEP here does NOT inherit Gate 2 -- it needs its own accuracy run in the SAME session.
