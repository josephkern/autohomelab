#!/usr/bin/env bash
# TUNED VARIANT (n3-async-sched) — base: runbooks/unsloth/Qwen3.8-27B-NVFP4/20260818_mtp-n3_tuned.sh
# Deltas vs base: ADD --async-scheduling
# Hypothesis: motivated by a MEASURED py-spy stack, not a guess. On a healthy MTP n=3 serve
#   mid-c16-bench, 3/3 samples showed the engine core inside spec-decode DRAFT METADATA
#   construction (build_for_drafting -> build_per_group_and_layer_attn_metadata -> flashinfer
#   build -> seq_lens_cpu) on one core pegged at ~100%. That is real per-step CPU work on the
#   critical path, and MTP multiplies it: n=3 rebuilds this metadata every draft step.
#   vLLM documents --async-scheduling as avoiding 'gaps in GPU utilization, leading to better
#   latency and throughput' -- i.e. overlapping the CPU-side step with GPU execution, which is
#   exactly the shape of what the stack shows.
# NOTE I previously argued this lever was worth <=3% on the theory that the pegged core was
#   spin-waiting on the GPU. The stack FALSIFIED that theory; this is the retest.
# Secondary read: if this helps at c16, it also offers a mechanism for the depth turnover
#   (n=4 wins c1 28.99 vs 25.64 but loses c16) without needing acceptance decay to explain it.
# Risk: LOW-MED. Scheduling only, numerically identical, so Gate 2 carries over. Historically
#   async scheduling has had rough edges with spec-decode, so serve_fail is the failure mode
#   run_experiment gates on.
