#!/usr/bin/env bash
# TUNED VARIANT (orch-profile) — base: runbooks/unsloth/Qwen3.8-27B-NVFP4/VLLM-27-unsloth_Qwen3.8-27B_NVFP4_final.sh
# Deltas vs base: --max-model-len 8192 -> 262144, AND ADD --enable-prefix-caching.
# NOT A TUNE-LOOP CANDIDATE — this is a DEPLOYMENT PROFILE, so it deliberately changes two
#   things at once and is NOT subject to the one-change rule or the >3% KEEP rule. Its purpose
#   is to measure throughput at ORCHESTRATOR-SCALE CONTEXT, a workload no row in the 313-row
#   results journal covers: every existing shape is 512 or 4096 prompt tokens.
# Target workload: one long-context orchestrator (>66k, ideally ~128k) plus many short-context
#   subagents. --max-model-len is a CEILING, not a reservation — vLLM allocates KV per token as
#   sequences grow — so setting it to native 262144 costs nothing for short sequences and simply
#   permits the orchestrator to grow. Budget is the shared pool: 283,989 live tokens at util 0.5
#   (engine-reported), e.g. a 128k orchestrator leaves room for ~18 concurrent 8k subagents.
# --enable-prefix-caching is included because it is OFF BY DEFAULT on hybrid models and this is
#   the workload it exists for: an orchestrator resending a growing context re-prefills the whole
#   thing every turn without it. Our tune loop measured it at -0.54% and discarded it, but that
#   was on GuideLLM synthetic prompts which share NO prefix — the benchmark is structurally blind
#   to the effect. NOTE this measurement is likewise blind: synthetic prompts get no cache hits,
#   so the numbers below are the WORST CASE and real orchestrator turns should beat them.
# Expected: substantially lower tok/s than the 512/256 headline (206 c16), because attention and
#   KV traffic dominate at 64k+. The coder shape (4096 prompt) already drops c16 206 -> 144.
