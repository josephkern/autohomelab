#!/usr/bin/env bash
# TUNED VARIANT (n3-schedtok8k) — base: runbooks/unsloth/Qwen3.8-27B-NVFP4/20260818_mtp-n3_tuned.sh
# Deltas vs base: ADD --max-num-scheduled-tokens 8192
# Hypothesis: the ONE loose thread from the Inferact campaign. Every spec-decode serve warns
#   'max_num_scheduled_tokens is set to 2048 based on the speculative decoding settings ... consider
#   increasing max_num_batched_tokens'. On Inferact we raised --max-num-batched-tokens to 16384 and
#   got +0.11% (noise), which never actually tested the quantity the warning NAMES. 0.27.1 exposes
#   --max-num-scheduled-tokens directly ('usually equal to max_num_batched_tokens, but can be smaller
#   when the model appends tokens into the batch, such as speculative decoding'). This sets it to the
#   default batched-token budget (8192) so draft slots stop crowding real work at c16.
# Risk: LOW. Scheduling only, numerically identical, Gate 2 carries over.
