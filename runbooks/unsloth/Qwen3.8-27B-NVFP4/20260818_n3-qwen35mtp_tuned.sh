#!/usr/bin/env bash
# TUNED VARIANT (n3-qwen35mtp) — base: runbooks/unsloth/Qwen3.8-27B-NVFP4/20260818_mtp-n3_tuned.sh
# Deltas vs base: speculative-config method "mtp" -> "qwen3_5_mtp" (n stays 3)
# Hypothesis: 0.27.1's --spec-method list carries an ARCH-SPECIFIC qwen3_5_mtp alongside the generic
#   mtp we have used in both campaigns. Never tested. This arch is 48/64 Gated DeltaNet with a single
#   MTP head re-run N times; if the specialised path handles linear-attention state across draft steps
#   properly, acceptance should rise at the SAME depth, which is free throughput. If it is an alias,
#   expect flat -- still worth knowing, because it closes the question permanently.
# Risk: LOW. Greedy-lossless, no download, no memory change; failure mode is serve_fail.
