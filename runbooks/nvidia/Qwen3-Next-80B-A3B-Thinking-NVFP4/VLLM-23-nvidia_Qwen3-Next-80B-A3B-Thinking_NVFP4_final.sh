#!/usr/bin/env bash
# FINAL (campaign-selected) — promoted from runbooks/nvidia/Qwen3-Next-80B-A3B-Thinking-NVFP4/20260622_mtp-n1_tuned.sh on 2026-06-23.
# Result: mtp-n1 (native qwen3_next MTP, num_spec=1): chat c16 256.85->295.14 (+14.9%), c1 42.3->56.23 (+33%); gsm8k flexible 81=81 lossless; smoke PASS. Patched image vllm-openai:0.23.0-qwen3nextmtp-fix + mtp.* checkpoint patch.
# Canonical config to serve. The *_tuned.sh experiment artifacts are kept intact as record.
# TUNED VARIANT (mtp-n1) — base: runbooks/nvidia/Qwen3-Next-80B-A3B-Thinking-NVFP4/baseline.sh
# Deltas vs base: + `--speculative-config '{"method":"qwen3_next_mtp","num_speculative_tokens":1}'`.
# Hypothesis: native single-token MTP draft. This model has ONE nextn layer (mtp.layers.0), so
#   num_spec=1 fits it (vLLM warns num_spec>1 reuses the single MTP layer → lower acceptance). This
#   was the DOMINANT lever on the sibling Instruct-NVFP4 (+12.5% c16 / +30% c1). Greedy-lossless.
