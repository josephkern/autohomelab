#!/usr/bin/env bash
# TUNED VARIANT (mtp-async) — base: runbooks/nvidia/NVIDIA-Nemotron-Labs-3-Puzzle-75B-A9B-NVFP4/20260709_mtp-n1_tuned.sh
# Deltas vs base: + `--async-scheduling` (overlap the scheduler step with GPU execution).
# Hypothesis: card recommends async-scheduling; it hides host-side scheduling latency behind GPU work,
#   which can add a few % at batched concurrency. Stacks on the mtp-n1 winner (c16 169.3). RISK: async-
#   scheduling has had compatibility friction with spec-decode in some vLLM builds -> may serve_fail or
#   regress; KEEP only if median c16 > 169.3 by >3% AND smoke passes.
...
VLLM_FLAGS=(
  ...
  --speculative-config '{"method":"mtp","num_speculative_tokens":1}'   # native MTP spec decode (n=1, from mtp-n1)
  --async-scheduling                                                   # DELTA: overlap scheduler with GPU exec
