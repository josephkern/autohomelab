#!/usr/bin/env bash
# FINAL (campaign-selected) — promoted from runbooks/nvidia/Qwen3-Next-80B-A3B-Instruct-NVFP4/20260621_mtp-n1_tuned.sh on 2026-06-21.
# Result: MTP winner (num_spec=1) on patched 0.23.0 (vLLM #35031 3-patch unblock). +12.5% chat c16 (237→267), +30% c1, +17-26% coder; lift across ALL levels/shapes. Greedy-lossless (gsm8k 94 vs 96, mmlu_pro 71.3 vs 71.9 = noise). smoke 3/3. Requires vllm-openai:0.23.0-qwen3nextmtp-fix + checkpoint mtp.* quant-ignore patch.
# Canonical config to serve. The *_tuned.sh experiment artifacts are kept intact as record.
# TUNED VARIANT (mtp-n1) — base: runbooks/nvidia/Qwen3-Next-80B-A3B-Instruct-NVFP4/baseline.sh
# Deltas vs base: + `--speculative-config '{"method":"qwen3_next_mtp","num_speculative_tokens":1}'`.
# Hypothesis: native single-token MTP draft. This model has ONE nextn layer, so num_spec=1 fits it
#   (vLLM warns num_spec>1 reuses the single MTP layer → lower acceptance). PRIMARY MTP candidate;
#   compare vs mtp-n2 (num_spec=2). Greedy-lossless. Community reports ~+60% on this exact checkpoint.
# Requires the patched image (vLLM #35031, 3 patches) — see baseline.sh / image.lock.
...
VLLM_FLAGS=(
  ...
  --max-model-len 8192
  --speculative-config '{"method":"qwen3_next_mtp","num_speculative_tokens":1}'   # DELTA: native MTP, single draft token
