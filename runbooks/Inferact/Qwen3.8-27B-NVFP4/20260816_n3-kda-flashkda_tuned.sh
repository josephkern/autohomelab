#!/usr/bin/env bash
# TUNED VARIANT (n3-kda-flashkda) — base: runbooks/Inferact/Qwen3.8-27B-NVFP4/20260816_mtp-n3_tuned.sh
# Deltas vs base: ADD --kda-prefill-backend flashkda (on top of the n=3 winner)
# Hypothesis: 0.27-ONLY FLAG (does not exist on 0.25.0). The serve log shows this model running
#   "Using Triton/FLA GDN prefill kernel (requested=auto, head_k_dim=128)" -- i.e. we are on the
#   Triton path by default and have never tried the alternative. 48 of 64 layers are Gated DeltaNet,
#   so the GDN prefill kernel is a large share of prefill work. flashkda is prefill-side, so it
#   composes with MTP (unlike --use-replayssm, which the help documents as "standard
#   (non-speculative) decode only" and is therefore incompatible with our winner).
#   Expect the effect on the chat shape (512 prompt) to be modest; the coder shape (4096) is where
#   a prefill kernel should show, so re-check coder at finalize even if chat c16 is flat.
