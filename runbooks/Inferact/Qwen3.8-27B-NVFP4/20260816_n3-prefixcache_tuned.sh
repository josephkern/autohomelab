#!/usr/bin/env bash
# TUNED VARIANT (n3-prefixcache) — base: runbooks/Inferact/Qwen3.8-27B-NVFP4/20260816_mtp-n3_tuned.sh
# Deltas vs base: ADD --enable-prefix-caching (on top of the n=3 winner)
# Hypothesis: shipped by both the Spark Arena recipe and our 35B _final. GuideLLM synthetic prompts are
#   randomized, so shared-prefix reuse should be near zero in THIS benchmark -- the honest
#   expectation is c16 flat/noise. Running it anyway because (a) it is cheap, (b) a REGRESSION would
#   be informative: on a hybrid model, prefix caching forces mamba_cache_mode 'align' and changes
#   how SSM state is checkpointed, so it is not free the way it is on a pure-attention model.
#   If flat, KEEP it anyway at promote time for real serving traffic, documented as bench-neutral.
