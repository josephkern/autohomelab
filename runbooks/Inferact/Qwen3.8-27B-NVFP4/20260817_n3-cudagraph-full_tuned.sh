#!/usr/bin/env bash
# TUNED VARIANT (n3-cudagraph-full) — base: runbooks/Inferact/Qwen3.8-27B-NVFP4/20260816_mtp-n3_tuned.sh
# Deltas vs base: ADD --compilation-config '{"cudagraph_mode":"FULL_AND_PIECEWISE"}'
# Hypothesis: the serve log for the n=3 winner captures graphs as "mixed prefill-decode, PIECEWISE",
#   i.e. we are on piecewise CUDA graphs and have never tried full ones. Piecewise graphs relaunch
#   per-segment every step, so the CPU-side launch overhead is paid once per segment per token --
#   and SPEC DECODE MULTIPLIES THAT, because each accept/reject cycle runs the model N+1 times per
#   emitted token. A bandwidth-bound decode leaves the CPU as the only place left to win, and this
#   is the flag that addresses it. FULL_AND_PIECEWISE keeps piecewise for prefill (needed for
#   chunked/mixed batches) while using full graphs for pure-decode steps, which is exactly the
#   shape of the c16 objective. Valid enum values confirmed against the pinned image:
#   NONE, PIECEWISE, FULL, FULL_DECODE_ONLY, FULL_AND_PIECEWISE.
# Risk: LOW-MED. Numerically identical (graph capture changes scheduling, not math), so no Gate-2
#   exposure. Costs extra capture time at load and a little memory for the captured graphs; the
#   real risk is a capture failure on this hybrid arch, which surfaces as serve_fail at startup.
