#!/usr/bin/env bash
# TUNED VARIANT (n3-util06) — base: runbooks/Inferact/Qwen3.8-27B-NVFP4/20260816_mtp-n3_tuned.sh
# Deltas vs base: --gpu-memory-utilization 0.5 -> 0.6 (on top of the n=3 winner)
# Hypothesis: the Spark Arena GB10 recipe runs 0.6. More pool = more KV = more resident sequences at c16.
#   BUT baseline already had 45x concurrency headroom at ctx 8192, so KV is not the binding
#   constraint and this may be pure noise. UNIFIED-MEMORY RISK (AGENTS.md): the pool is shared with
#   the OS, so high util destabilises the BOX, not just the GPU -- watch for swap/thrash, and the
#   c1 sentinel dropping between benches is the tell (that is how FF711's over-commit showed up).
