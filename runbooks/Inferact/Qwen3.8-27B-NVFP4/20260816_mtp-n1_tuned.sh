#!/usr/bin/env bash
# TUNED VARIANT (mtp-n1) — base: runbooks/Inferact/Qwen3.8-27B-NVFP4/baseline.sh
# Deltas vs base: ADD --speculative-config '{"method":"mtp","num_speculative_tokens":1}'
# Hypothesis: SHALLOWEST spec depth, and the one this arch is most likely to prefer. The model has
#   mtp_num_hidden_layers=1, and vLLM warns at startup that num_speculative_tokens>1 "will run
#   multiple times of forward on same MTP layer, which may result in lower acceptance rate" — so
#   n=1 is the only depth that uses the head as trained, with no acceptance decay from re-running it.
#   FF711 independently found the depth optimum SHALLOWS as batch grows, and our objective is c16.
#   Bracket member: n=1 vs n=2 vs n=3, each a one-change variant off the same baseline.
