#!/usr/bin/env bash
# TUNED VARIANT (linear-marlin) on vLLM 0.23.0 — base: runbooks/RedHatAI/gemma-4-31B-it-NVFP4/baseline.sh
# Deltas vs base: + --linear-backend marlin (vs auto=FlashInferCutlassNvFp4)
# Hypothesis: Compare the conservative marlin NVFP4 GEMM vs the native cutlass path auto picks. 8B found native > marlin; confirm on dense gemma. Numeric-risky -> eval if it wins.
