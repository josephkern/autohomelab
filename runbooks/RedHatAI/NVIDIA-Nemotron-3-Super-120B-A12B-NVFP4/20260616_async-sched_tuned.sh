#!/usr/bin/env bash
# TUNED VARIANT (async-sched) — base: runbooks/RedHatAI/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4/baseline.sh
# Deltas vs base: + `--async-scheduling`.
# Hypothesis: overlaps the scheduler with GPU execution (blog Fig-4 throughput lever); may lift batched
#   c16 by hiding scheduling latency between decode steps.
