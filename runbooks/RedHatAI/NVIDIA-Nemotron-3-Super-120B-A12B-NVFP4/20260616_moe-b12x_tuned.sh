#!/usr/bin/env bash
# TUNED VARIANT (moe-b12x) — base: runbooks/RedHatAI/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4/baseline.sh
# Deltas vs base: + `--moe-backend flashinfer_b12x` (vs the auto-selected MoE backend).
# Hypothesis: 0.23.0 synced FlashInfer b12x FP4 MoE GEMM for SM120/121; may beat auto on the LatentMoE
#   expert GEMMs. Risk: b12x serve_failed on the 8B/35B campaigns — first real test on 0.23.0.
...
VLLM_FLAGS=(
  ...
  --moe-backend flashinfer_b12x   # DELTA: FP4 MoE GEMM (vs auto)
