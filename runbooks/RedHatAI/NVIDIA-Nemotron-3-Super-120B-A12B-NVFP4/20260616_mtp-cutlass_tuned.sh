#!/usr/bin/env bash
# TUNED VARIANT (mtp-cutlass) — base: runbooks/RedHatAI/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4/baseline.sh
# Deltas vs base: + MTP spec-decode AND `--moe-backend flashinfer_cutlass` — STACK the two queue winners.
# Hypothesis: MTP (+22.8% c16 solo) and explicit cutlass MoE (+6.8% solo) are orthogonal (decode draft
#   vs expert GEMM) → expect ~additive, ~+30% over baseline. This is the promotion candidate if it holds.
...
VLLM_FLAGS=(
  ...
  --speculative-config '{"method":"mtp","num_speculative_tokens":1}'   # DELTA 1: native MTP spec decode (+22.8% solo)
  --moe-backend flashinfer_cutlass   # DELTA 2: explicit CUTLASS FP4 MoE (+6.8% solo)
