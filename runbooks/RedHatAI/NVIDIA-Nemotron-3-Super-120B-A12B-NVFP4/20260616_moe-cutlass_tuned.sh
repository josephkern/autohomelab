#!/usr/bin/env bash
# TUNED VARIANT (moe-cutlass) — base: runbooks/RedHatAI/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4/baseline.sh
# Deltas vs base: + `--moe-backend flashinfer_cutlass` (explicit, vs auto).
# Hypothesis: pins the CUTLASS FP4 MoE path; confirms whether auto already selects it (35B found
#   auto==cutlass) or differs for the LatentMoE arch. Low-risk characterization of the MoE backend.
...
VLLM_FLAGS=(
  ...
  --moe-backend flashinfer_cutlass   # DELTA: explicit CUTLASS FP4 MoE (vs auto)
