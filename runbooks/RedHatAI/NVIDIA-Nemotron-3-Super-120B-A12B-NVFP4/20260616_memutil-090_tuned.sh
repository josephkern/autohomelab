#!/usr/bin/env bash
# TUNED VARIANT (memutil-090) — base: runbooks/RedHatAI/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4/baseline.sh
# Deltas vs base: `--gpu-memory-utilization 0.85 -> 0.90`.
# Hypothesis: +5% of the unified pool → more KV blocks → more concurrent decode slots at c16/c32 (the
#   batched throughput is KV-block-bound here). Risk: less OS headroom on the shared pool — watch stability.
...
VLLM_FLAGS=(
  ...
  --gpu-memory-utilization 0.90   # DELTA: 0.85 -> 0.90 (more KV headroom; less OS slack on unified)
