#!/usr/bin/env bash
# TUNED VARIANT (maxseqs-4) — base: runbooks/RedHatAI/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4/baseline.sh
# Deltas vs base: + `--max-num-seqs 4` (the blog DGX-Spark recommendation).
# Hypothesis: CHARACTERIZATION, not a win — caps in-flight decode at 4, so c16/c32 AGGREGATE should DROP
#   (only 4 streams batched). Quantifies the per-stream-latency vs aggregate-throughput tradeoff the blog
#   optimizes for. Expected to LOSE the c16 objective vs baseline.
...
VLLM_FLAGS=(
  ...
  --max-num-seqs 4                # DELTA: blog's per-stream cap (characterization; expected c16 drop)
