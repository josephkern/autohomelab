#!/usr/bin/env bash
# TUNED VARIANT (moe-auto) on vLLM 0.23.0 — base: runbooks/RedHatAI/Qwen3.6-35B-A3B-NVFP4/baseline.sh
# Deltas vs base: --moe-backend flashinfer_cutlass -> auto (native sm_12x MoE GEMM path)
# Hypothesis: auto may pick the faster native Blackwell b12x/cutedsl MoE kernel, mirroring the dense native-FP4 win. Numeric-risky (GEMM path) -> eval mmlu.
