#!/usr/bin/env bash
# TUNED VARIANT (moe-auto) — base: runbooks/RedHatAI/Qwen3.6-35B-A3B-NVFP4/20260704_v0.24.0_baseline.sh
# Deltas vs base: --moe-backend flashinfer_cutlass -> auto
# Hypothesis: on 0.24 the MoE path is newly enabled on SM120 — auto may now pick a faster native
#   Blackwell MoE kernel than the pinned flashinfer_cutlass. Numeric-risky (GEMM path) -> eval mmlu.
