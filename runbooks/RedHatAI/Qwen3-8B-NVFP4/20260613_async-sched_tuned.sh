#!/usr/bin/env bash
# TUNED VARIANT (async-sched) — base: runbooks/RedHatAI/Qwen3-8B-NVFP4/20260613_native-fp4_tuned.sh
# Deltas vs base: + `--async-scheduling`.
# Hypothesis: overlaps CPU-side scheduling with GPU execution, removing gaps in GPU utilization
#   between steps → should lift steady-state batched (c16) throughput. Low risk (used in the prior
#   GB10 MoE serve scripts).
