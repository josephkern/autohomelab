#!/usr/bin/env bash
# TUNED VARIANT (kv-fp8) — base: runbooks/RedHatAI/Qwen3-8B-NVFP4/20260613_native-fp4_tuned.sh
# Deltas vs base: + `--kv-cache-dtype fp8_e4m3` (was auto/bf16).
# Hypothesis: fp8 KV halves KV bytes/token -> more KV headroom + less KV bandwidth per step, which
#   can lift batched (c16) throughput. NOTE: FlashAttention can't do fp8 KV on sm_121, so this also
#   forces the attention backend to FlashInfer/Triton — a coupled change to watch in the logs.
