#!/usr/bin/env bash
# TUNED VARIANT (mnbt-16384) — base: runbooks/RedHatAI/Qwen3-Coder-Next-NVFP4/baseline.sh
# Deltas vs base: + `--max-num-batched-tokens 16384` (V1 online default ~8192).
# Hypothesis: chunked-prefill co-schedules more prefill alongside decodes at c16/c32 → modest
#   batched-throughput lift on a bandwidth-bound MoE. Flat at c1, tapers as bandwidth saturates.
#   KV fits easily (chat 16×768 tok is tiny vs the 0.5-util pool). candidate-queue #1 (low risk).
...
VLLM_FLAGS=(
  ...
  --max-model-len 8192
  --max-num-batched-tokens 16384   # DELTA: co-schedule more prefill w/ decode (default ~8192)
